...
__doc__ = """
  ...
  * Ji Zhao, Deyu Meng. FastMMD: Ensemble of Circular Discrepancy for Efficient
 Two-Sample Test. Neural Computation, 2015.
"""
...
#         print(XX.shape)
        T = FWHT(XX)
        T_m = pd.read_csv(
            '/Users/kellanfluette/dev/fastfood/samples/' +
            'matlab_fastfood/Fastfood/fwht-xx-' + str(ii + 1) +
            '.csv', header=None)
        print('T =', T, ', T_m =', T_m)
#         print('PI =',PI)
        T = T[PI, :]
...
def ff_kernel(para, sgm, x1, x2):
    # TODO use the precomputed Gram matrix rather than calculating K(x,y)
    # individually
    X = np.hstack((x1, x2))
