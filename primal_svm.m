function [sol,b,obj] = primal_svm(X,Y,K,linear,lambda,opt)
...
% http://olivier.chapelle.cc/primal/primal_svm.m
% Modified 30 Jul 2015 by Kellan Fluette <fluetteka@g.cofc.edu>
  ...
  if  linear 
    if isempty(X), error('X can not be empty'); end;
    [n,d] = size(X);
    ...
    if isempty(K), error('K can not be empty'); end;
    n = size(Y,1);
